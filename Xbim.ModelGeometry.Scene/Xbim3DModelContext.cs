...
namespace Xbim.ModelGeometry.Scene
{
    ...
    public class Xbim3DModelContext : ICanLog
    {
        ...
        /// <summary>
        /// Gets the curves optionally generated by <see cref="CreateContext()" /> with entity label and vertices.
        /// </summary>
        public IReadOnlyDictionary<int, IReadOnlyList<XbimPoint3D>> Curves { get; private set; } = new Dictionary<int, IReadOnlyList<XbimPoint3D>>();

        /// <summary>
        /// Gets the points optionally generated by <see cref="CreateContext()" /> with entity label and vertices.
        /// </summary>
        public IReadOnlyDictionary<int, XbimPoint3D> Points { get; private set; } = new Dictionary<int, XbimPoint3D>();
        ...
        /// <param name="generateCurvesAndPoints">When <c>true</c> curves and points.</param>
        /// <returns></returns>
        public bool CreateContext(ReportProgressDelegate progDelegate = null, bool adjustWcs = true, bool generateBREPs = false,
            ...
            bool generateCurvesAndPoints = false)
        {
            ...
            using (var geometryTransaction = geometryStore.BeginInit())
            {
                ...
                using (var contextHelper = new XbimCreateContextHelper(this, _model, _contexts, _logger))
                {
                    ...
                    if (generateCurvesAndPoints)
                    {
                        progDelegate?.Invoke(-1, "GetCurves");
                        _logger.LogTrace("Starting GetCurves");
                        GetCurvesAndPoints(contextHelper);
                        progDelegate?.Invoke(101, "GetCurves");
                    }
                }
        ...
        private void GetCurvesAndPoints(XbimCreateContextHelper contextHelper)
        {
            ...
            var points = new Dictionary<int, XbimPoint3D>();
                ...
                foreach (var representation in product.Representation.Representations)
                {
                    ...
                    foreach (var item in representation.Items)
                    {
                        switch (item)
                        {
                            case IIfcMappedItem mappedItem:
                                foreach (var innerItem in mappedItem.MappingSource?.MappedRepresentation?.Items)
                                {
                                    switch (innerItem)
                                    {
                                        ...
                                        case IIfcCartesianPoint cartesianPoint:
                                            if (!points.ContainsKey(cartesianPoint.EntityLabel))
                                            {
                            ...
                            default:
                                _logger.LogWarning("Unknown entity type {0}.", item.GetType().FullName);
                                break;
                        }
                    }
                }
            ...
            Curves = curves.ToDictionary<KeyValuePair<int, List<XbimPoint3D>>, int, IReadOnlyList<XbimPoint3D>>(
                polyline => polyline.Key,
                polyline => polyline.Value);
            Points = points;
        }

        private static XbimPoint3D TransformCartesianPoint(IIfcCartesianPoint point, XbimMatrix3D transform)
        {
            return (int)point.Dim switch
            {
                2 => XbimPoint3D.Multiply(new XbimPoint3D(point.X, point.Y, 0), transform),
                3 => XbimPoint3D.Multiply(Ifc.IIfcCartesianPointExtensions.ToXbimPoint3D(point), transform),
